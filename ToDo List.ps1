...
$todoFolderPath = Join-Path $scriptPath "todo"
$settingsFilePath = Join-Path $todoFolderPath "settings.ini"
if (-not (Test-Path $todoFolderPath)) { New-Item -ItemType Directory -Path $todoFolderPath | Out-Null }
...
function Set-ControlsState($enabled) {
    $listView.Enabled = $textBox.Enabled = $addButton.Enabled = $removeButton.Enabled = $comboBox.Enabled = $deleteListButton.Enabled = $setDefaultListButton.Enabled = $enabled
...
$form.Add_KeyDown({ if ($_.KeyCode -eq "Delete") { Remove-SelectedTasks } })
